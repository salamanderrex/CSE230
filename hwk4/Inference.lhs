...
\begin{code}
env   = Map.fromList [ (EV "plus", Forall [] $ tArrs [TInt, TInt, TInt]) , (EV "ite" , Forall [TV "a"] $ tArrs [TBool, TVbl (TV "a"), TVbl (TV "a"), TVbl (TV "a")])]
...
ti env (e1 `ECons` e2) = do
                           (s1,t1) <- ti env e1
                           (s2,t2) <- tiList (apply s1 env) e2
                           s3 <- mgu (apply s2 t1) t2
                           return (s3 `after` s2 `after` s1, apply s3 (TList t2))
ti env (EIsNil e)      = do (s,t) <- tiList env e
                            return (s,TBool)
                             
ti env (EDcons e)      = do (s,t) <- tiList env e
...
tiList env e = do tv <- freshTVbl "a"
                  (s1,t1) <- ti env e
                  s2 <- mgu (TList tv) t1
                  let s = s2 `after` s1
                  return (s, apply s tv)
...
test' e = case typeInference (tenv) e of
           Left err  ->  putStrLn $ "error: " ++ err
           Right t   ->  putStrLn $ show e ++ " :: " ++ show t
...
prType (t1 `TCom` t2) = PP.text "(" PP.<+> prType t1 PP.<+> PP.text "," PP.<+> prType t2 PP.<+> PP.text")"
