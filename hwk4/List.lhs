...
\begin{code}
{-@ measure innerSize :: List( List a) -> Int
            innerSize Emp   = 0
            innerSize ( (:+:) x xs) = size x + innerSize xs
@-}
{-@ type ListY a XS = {v: List a  | size v == innerSize XS} @-}
{-@ concat :: l:(List (List a)) -> ListN a {innerSize l} @-}
concat Emp = Emp
concat (x :+: xs) = foldr'  (concat xs) x

{-@ foldr' :: l1: List a -> l2:List a -> {v: List a| size v = size l1 + size l2} @-}
foldr'  b Emp        = b
